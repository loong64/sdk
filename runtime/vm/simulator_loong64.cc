...
static int32_t LA_BranchOff26(uint32_t instr) {
  return static_cast<int32_t>(instr << 6) >> 6;
}
...
void Simulator::ExecuteNoTrace() {
  while (pc_ != kEndSimulatingPC) {
    ...
    if (op6 == 0x00) {
      ...
      if (func4 >= 8) {
        ...
        switch (func4) {
          ...
          case 0x9: set_xreg(static_cast<Register>(rd), static_cast<uint64_t>(val_rj) < static_cast<uint64_t>(static_cast<uint32_t>(imm)) ? 1 : 0); break;  // sltui
        ...
        switch (op17) {
          ...
          case 71:  // mod.du: 0x00238000
            ...
            break;
          default:
            Fault("Unknown ALU 3R instruction");
        }
        }
      }
    } else if ((instr & 0xfc000000) == 0x28000000) {
    ...
    } else if ((instr & 0xfc000000) == 0x2A000000) {
    ...
    } else if ((instr & 0xfc000000) == 0x29000000) {
      ...
      bool taken = false;
      switch (op6) {
        case 0x16: taken = (val_rj == val_rk); break;
        case 0x17: taken = (val_rj != val_rk); break;
        case 0x18: taken = (val_rj < val_rk); break;
        case 0x19: taken = (val_rj >= val_rk); break;
        case 0x1a: taken = (static_cast<uint64_t>(val_rj) < static_cast<uint64_t>(val_rk)); break;
        case 0x1b: taken = (static_cast<uint64_t>(val_rj) >= static_cast<uint64_t>(val_rk)); break;
      }
      ...
      if (sub == 0) {
        ...
        cur = (cur & 0xfff) | (static_cast<uint64_t>(static_cast<uint32_t>(si20)) << 32);
        set_xreg(static_cast<Register>(rd), static_cast<int64_t>(cur));
      } else {
        Fault("lu52id not implemented");
      }
    } else if (op6 == 0x04) {
      ...
      if ((instr & 0x00200000) == 0) {
        // fld.s
        set_fregs(static_cast<FRegister>(rd), MemoryRead<float>(addr, static_cast<Register>(rj)));
      } else if ((instr & 0x00200000) == 0x00200000) {
        // fld.d
        set_fregd(static_cast<FRegister>(rd), MemoryRead<double>(addr, static_cast<Register>(rj)));
      } else if ((instr & 0x04000000) == 0x04000000 && (instr & 0x00200000) == 0) {
        // fst.s
        MemoryWrite<float>(addr, get_fregs(static_cast<FRegister>(rd)), static_cast<Register>(rj));
      } else {
        // fst.d
        MemoryWrite<double>(addr, get_fregd(static_cast<FRegister>(rd)), static_cast<Register>(rj));
      }
    ...
    if ((op10_fp & 0x3fc) == 0x010) {
      ...
      float result = 0.0f;
      if (op10_fp == 0x010) result = s1 + s2;
      else if (op10_fp == 0x008) result = s1 - s2;
      else if (op10_fp == 0x018) result = s1 * s2;
      else if (op10_fp == 0x020) result = s1 / s2;
      else Fault("Unknown FP single op");
      ...
      double result = 0.0;
      if (op10_fp == 0x014) result = d1 + d2;
      else if (op10_fp == 0x00c) result = d1 - d2;
      else if (op10_fp == 0x01c) result = d1 * d2;
      else if (op10_fp == 0x024) result = d1 / d2;
      else Fault("Unknown FP double op");
      ...
      if (is_double) {
        ...
        double d2 = get_fregd(static_cast<FRegister>(rk));
        switch (cond) {
          case 0x0: fcrcmp_result_ = (d1 < d2) ? 1 : (d1 > d2 ? 2 : (d1 == d2 ? 0x40 : 0x10)); break;  // CAF/SAF/CMP
          case 0x1: fcrcmp_result_ = (d1 < d2) ? 1 : (d1 > d2 ? 2 : 0x10); break;  // CUN/SAF
          case 0x2: fcrcmp_result_ = (d1 < d2) ? 1 : (d1 > d2 ? 2 : 0); break;    // CUEQ
          case 0x8: fcrcmp_result_ = (d1 <= d2) ? (d1 < d2 ? 1 : 0x40) : 2; break;  // CLE
          case 0xc: fcrcmp_result_ = (d1 == d2) ? 0x40 : ((d1 < d2) ? 1 : 2); break;  // CEQ
          case 0xe: fcrcmp_result_ = (d1 == d2 || d1 < d2) ? ((d1 == d2) ? 0x40 : 1) : 2; break;  // CLE (UN)
          default: break;
        }
      } else {
        ...
        switch (cond) {
          case 0x0: fcrcmp_result_ = (f1 < f2) ? 1 : (f1 > f2 ? 2 : (f1 == f2 ? 0x40 : 0x10)); break;
          case 0x1: fcrcmp_result_ = (f1 < f2) ? 1 : (f1 > f2 ? 2 : 0x10); break;
          case 0x2: fcrcmp_result_ = (f1 < f2) ? 1 : (f1 > f2 ? 2 : 0); break;
          case 0x8: fcrcmp_result_ = (f1 <= f2) ? (f1 < f2 ? 1 : 0x40) : 2; break;
          case 0xc: fcrcmp_result_ = (f1 == f2) ? 0x40 : ((f1 < f2) ? 1 : 2); break;
          case 0xe: fcrcmp_result_ = (f1 == f2 || f1 < f2) ? ((f1 == f2) ? 0x40 : 1) : 2; break;
          default: break;
        }
      }
      ...
      if (op10_fp == 0x088) {
        set_fregd(static_cast<FRegister>(rd), static_cast<double>(get_fregs(static_cast<FRegister>(rj))));
      } else {
        set_fregs(static_cast<FRegister>(rd), static_cast<float>(get_fregd(static_cast<FRegister>(rj))));
      }
    } else if ((op10_fp & 0x3f8) == 0x090) {
      // ftintrz: w.s=0x090, w.d=0x094, l.s=0x098, l.d=0x09c
      if (op10_fp == 0x090) set_fregs(static_cast<FRegister>(rd), static_cast<float>(static_cast<int32_t>(get_fregs(static_cast<FRegister>(rj)))));
      else if (op10_fp == 0x094) set_fregs(static_cast<FRegister>(rd), static_cast<float>(static_cast<int32_t>(get_fregd(static_cast<FRegister>(rj)))));
      else if (op10_fp == 0x098) set_fregd(static_cast<FRegister>(rd), static_cast<double>(static_cast<int64_t>(get_fregs(static_cast<FRegister>(rj)))));
      else set_fregd(static_cast<FRegister>(rd), static_cast<double>(static_cast<int64_t>(get_fregd(static_cast<FRegister>(rj)))));
    } else if ((op10_fp & 0x3f8) == 0x084) {
      // ffint: w.s=0x084, l.s=0x086, w.d=0x085, l.d=0x087
      if (op10_fp == 0x084) set_fregs(static_cast<FRegister>(rd), static_cast<float>(static_cast<int32_t>(static_cast<int32_t>(get_fregs(static_cast<FRegister>(rj))))));
      else if (op10_fp == 0x086) set_fregd(static_cast<FRegister>(rd), static_cast<double>(static_cast<int64_t>(static_cast<int32_t>(get_fregs(static_cast<FRegister>(rj))))));
      else if (op10_fp == 0x085) set_fregs(static_cast<FRegister>(rd), static_cast<float>(static_cast<int32_t>(static_cast<int64_t>(get_fregd(static_cast<FRegister>(rj))))));
      else set_fregd(static_cast<FRegister>(rd), static_cast<double>(static_cast<int64_t>(static_cast<int64_t>(get_fregd(static_cast<FRegister>(rj))))));
    } else {
      ...
      Fault("Unimplemented LoongArch instruction");
    }
...
type Simulator::MemoryRead(uintx_t address, Register base) {
  ...
  if (IsIllegalAddress(address)) {
    FATAL("Null pointer read at 0x%lx, pc=0x%lx", address, pc_);
  }
...
void Simulator::MemoryWrite(uintx_t address, type value, Register base) {
  if (IsIllegalAddress(address)) {
    FATAL("Null pointer write at 0x%lx, pc=0x%lx", address, pc_);
  }
