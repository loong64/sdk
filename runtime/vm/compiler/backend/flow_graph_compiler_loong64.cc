...
void FlowGraphCompiler::EmitInstanceCallJIT(const Code& stub,
  ...
  __ Load(TMP, compiler::FieldAddress(CODE_REG, entry_point_offset));
  __ Call(TMP);
...
void FlowGraphCompiler::EmitInstanceCallAOT(const ICData& ic_data,
  ...
  __ LoadUniqueObject(TMP, initial_stub, snapshot_behavior);
...
void FlowGraphCompiler::EmitMove(Location destination,
  ...
  if (source.IsRegister()) {
    ...
    if (destination.IsFpuRegister()) {
      __ MoveUnboxedDouble(destination.fpu_reg(), source.fpu_reg());
...
void ParallelMoveEmitter::EmitSwap(const MoveOperands& move) {
  ...
  if (source.IsRegister() && destination.IsRegister()) {
    ...
    __ fmov_d(FTMP, source.fpu_reg());
    __ fmov_d(source.fpu_reg(), destination.fpu_reg());
    __ fmov_d(destination.fpu_reg(), FTMP);
  } else if (source.IsFpuRegister() || destination.IsFpuRegister()) {
