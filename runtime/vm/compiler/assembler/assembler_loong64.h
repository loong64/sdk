...
class MicroAssembler : public AssemblerBase {
 public:
  ...
  // Atomic instructions: Load Linked / Store Conditional.
  void ll_w(Register rd, Register rj, intptr_t si14 = 0);
  void sc_w(Register rd, Register rj, intptr_t si14 = 0);
  void ll_d(Register rd, Register rj, intptr_t si14 = 0);
  void sc_d(Register rd, Register rj, intptr_t si14 = 0);
...
class Assembler : public MicroAssembler {
 public:
  ...
  void LoadMemoryValue(Register dst, Register base, int32_t offset) { LoadFromOffset(dst, base, offset, kWordBytes); }
  void StoreMemoryValue(Register src, Register base, int32_t offset) { StoreToOffset(src, base, offset, kWordBytes); }
  static bool IsSafe(const Object& object) { return true; }
  static bool IsSafeSmi(const Object& object) { return target::IsSmi(object); }
  void LslImmediate(Register dst, Register src, int32_t shift, OperandSize sz = kWordBytes) override;
  void AndImmediate(Register dst, Register src, target::word imm, OperandSize sz = kWordBytes) override;
  void LslImmediate(Register reg, int32_t shift, OperandSize sz = kWordBytes) override;
  void ArithmeticShiftRightImmediate(Register reg, int32_t shift, OperandSize sz = kWordBytes) override;
  ...
  void AddScaled(Register dst, Register base, Register index, ScaleFactor scale, int32_t disp) override;
  void CompareImmediate(Register reg, target::word imm, OperandSize width = kWordBytes) override;
  void AndImmediate(Register reg, target::word imm, OperandSize sz = kWordBytes) override;
  void LsrImmediate(Register dst, int32_t shift) override;
  void MulImmediate(Register dst, target::word imm, OperandSize sz = kWordBytes) override;
  ...
  void ExtractBitField(Register dst, Register src, intptr_t low_bit, intptr_t width) override;
  ...
  void InitializeHeaderUntagged(Register tags, Register object) {
    StoreToOffset(tags, object, target::Object::tags_offset());
  }
  void LoadClassIdMayBeSmi(Register result, Register object);
  void LoadTaggedClassIdMayBeSmi(Register result, Register object);
  void CompareClassId(Register object, intptr_t class_id, Register scratch = TMP);
  ...
  void OrImmediate(Register dst, Register src, target::word imm, OperandSize sz = kWordBytes);
  void OrImmediate(Register dst, intptr_t imm) { OrImmediate(dst, dst, imm); }
#if defined(DART_COMPRESSED_POINTERS)
  void LoadAcquireCompressed(Register dst, const Address& address) override;
  void LoadCompressed(Register dst, const Address& address) override;
  ...
  void MoveRegister(Register dst, Register src) override;
  ...
  void Call(intptr_t target_code_pool_index, CodeEntryKind entry_kind = CodeEntryKind::kMonomorphic);
  void Call(Address target);
  void Call(Register target);
  void Call(Label* label);
  void Call(const Code& target) { BranchLink(target); }
  ...
  void LoadImmediate(Register rd, intx_t value) override;

  void LoadSImmediate(FRegister reg, float imms);
  void LoadDImmediate(FRegister reg, double immd);


  void AddImmediate(Register rd, intx_t value) { AddImmediate(rd, rd, value); }
  ...
  using AssemblerBase::CompareWords;
  ...
  void MoveUnboxedDouble(FpuRegister dst, FpuRegister src) { fmv_d(dst, src); }

  void StoreUnboxedSimd128(FpuRegister src, Register base, int32_t offset) {
    // No single register SIMD on LoongArch.
    UNREACHABLE();
  }
  ...
  void InitializeHeader(Register tags, Register object) {
    st_d(tags, object, target::Object::tags_offset());
  }
  void LoadFromStack(Register dst, intptr_t depth);
  void StoreToStack(Register src, intptr_t depth);
  void CompareToStack(Register src, intptr_t depth);
  void SubRegisters(Register dest, Register src) { sub_d(dest, dest, src); }
  void StoreZero(const Address& address, Register temp = kNoRegister) { Store(ZR, address); }
 ...
 void JumpAndLink(const Code& code,
  ...
  void BranchLink(const Code& target,
                  ObjectPoolBuilderEntry::Patchability patchable =
                      ObjectPoolBuilderEntry::kNotPatchable,
                  CodeEntryKind entry_kind = CodeEntryKind::kNormal,
                  ObjectPoolBuilderEntry::SnapshotBehavior snapshot_behavior =
  ...
  void BranchLinkPatchable(const Code& target) {
    BranchLink(target, ObjectPoolBuilderEntry::kPatchable);
  }
