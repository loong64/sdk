 ...
 public:
  ...
  void LslImmediate(Register dst,
    ...
    if (sz == kFourBytes || sz == kUnsignedFourBytes) {
      // Shift the low word, then extend it according to the requested width.
      // A double-word shift would incorrectly incorporate bits 32 through 63
      // of src.
      slli_d(dst, src, 32);
      srli_d(dst, dst, 32);
      slli_d(dst, dst, shift);
      ExtendValue(dst, dst,
                  sz == kFourBytes ? kFourBytes : kUnsignedFourBytes);
    } else {
  ...
  void ArithmeticShiftRightImmediate(Register dst,
                                     ...
                                     OperandSize sz = kWordBytes) override {
    if (sz == kFourBytes) {
      ExtendValue(dst, src, kFourBytes);
      srai_d(dst, dst, shift);
    } else {
  ...
  void AndImmediate(Register dst,
    ...
    // Bitwise operations produce a zero-extended result for sub-word sizes.
    if (sz == kFourBytes || sz == kUnsignedFourBytes) {
      ExtendValue(dst, dst, kUnsignedFourBytes);
    } else if (sz == kTwoBytes || sz == kUnsignedTwoBytes) {
      ExtendValue(dst, dst, kUnsignedTwoBytes);
    } else if (sz == kByte || sz == kUnsignedByte) {
      ExtendValue(dst, dst, kUnsignedByte);
    }
  }
