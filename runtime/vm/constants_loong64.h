...
constexpr Register HEAP_BITS = S6;  // write_barrier_mask << 32 | heap_base >> 32
...
constexpr Register kAbiFirstPreservedCpuReg = FP;
constexpr Register kAbiLastPreservedCpuReg = S8;
...
constexpr FRegister kAbiFirstPreservedFpuReg = FS0;
constexpr FRegister kAbiLastPreservedFpuReg = FS7;
...
class Instr {
 public:
  ...
  // LoongArch register fields (bits[4:0]=rd, bits[9:5]=rj, bits[14:10]=rk).
  ...
  // LoongArch 6-bit opcode at bits[31:26].
  uint32_t opcode() const { return (encoding_ >> 26) & 0x3f; }

  // LoongArch branch offset decoding (21-bit for BEQZ/BNEZ).
  int32_t branch_offset21() const {
    int32_t lo = static_cast<int16_t>((encoding_ >> 10) & 0xffff);
    int32_t hi = encoding_ & 0x1f;
    return (hi << 16) | (lo & 0xffff);
  }

  // LoongArch branch offset decoding (16-bit for BEQ/BNE/BLT/BGE/BLTU/BGEU).
  int32_t branch_offset16() const {
    return static_cast<int16_t>((encoding_ >> 10) & 0xffff);
  }

  // LoongArch JIRL offset (16-bit at bits[25:10]).
  int32_t jirl_offset() const {
    return static_cast<int16_t>((encoding_ >> 10) & 0xffff);
  }

  // LoongArch FP register fields.
  FRegister frd() const { return static_cast<FRegister>(ExtractRd(encoding_)); }
  FRegister frj() const { return static_cast<FRegister>(ExtractRj(encoding_)); }
  FRegister frk() const { return static_cast<FRegister>(ExtractRk(encoding_)); }

  // Constants.
  // LoongArch break 0 instruction.
  static constexpr uint32_t kBreakPointInstruction = 0x002a0000;
  static constexpr uint32_t kInstrSize = 4;
  // LoongArch syscall 0 instruction (used for simulator redirect).
...
constexpr intptr_t kPreferredLoopAlignment = 1;
